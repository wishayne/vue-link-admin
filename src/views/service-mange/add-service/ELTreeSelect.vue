  ...
  methods: {
    onPopperVisibleChange(isVisible) {
      if (isVisible) {
        this.$nextTick(() => {
          ...
          const dropdownItem = dorpdownList[dorpdownList.length - 1]
          dropdownItem
            .getElementsByClassName('el-select-dropdown__list')[0]
            .appendChild(dom)
          const Tree = Vue.component('ElTree')
        ...
        const dropdownItem = dorpdownList[dorpdownList.length - 1]
        const parent = dropdownItem.getElementsByClassName(
          'el-select-dropdown__list'
        )[0]
        const treeNode = parent.getElementsByClassName('el-tree')[0]
    ...
    onRemoveItem(tagValue) {
      if (this.dropdownTree != null) {
        const checkedKeys = this.dropdownTree.getCheckedKeys()
